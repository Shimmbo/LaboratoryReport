...
CREATE TABLE `register` (
  ...
  KEY `Fk_Id_Teacher_Register_Teacher_idx` (`Id_Teacher`),
...
/*!50003 SET sql_mode              = 'NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER delete_register_teacher
...
CREATE TABLE `register_teacher` (
  ...
  CONSTRAINT `Fk_Id_Register_Register_Teacher` FOREIGN KEY (`Id_Register`) REFERENCES `register` (`Id_Register`) ON DELETE NO ACTION ON UPDATE NO ACTION,
...
INSERT INTO `register_teacher` VALUES (1,1,1),(2,2,2),(3,3,3),(5,5,5);
...
--
/*!50003 DROP PROCEDURE IF EXISTS `get_stuff_teacher_info` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8 */ ;
...
/*!50003 SET sql_mode              = 'NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `get_stuff_teacher_info`()
BEGIN
...
BEGIN
  SELECT r.Id_Register as id,
        ...
        END as end,
        r.Comments,
        IFNULL(CONCAT(t.Name, ' ',t.LastName, ' ', IFNULL(t.SecondLastName, ''), ' - ',
        l.Name, ' - ', s.Name, ' - ', s.Semester, ' - ', c.Name, ' - ' ,r.StudentsAssistanceNumber
        ,' - ',IFNULL(cc.Description, '')),
       CONCAT(tt.Name, ' ',tt.LastName, ' ', IFNULL(tt.SecondLastName, ''),
       ' - ', l.Name,' - ', r.StudentsAssistanceNumber, IFNULL(cc.Description,'')))as title,
        c.Color as color,
        ch.Id_Catalog_Hour as Id_Catalog_Hour,
        IFNULL(t.Id_Teacher,tt.Id_Teacher) as Id_Teacher,
        ...
        c.Id_Career,
        s.Semester
  FROM register r LEFT JOIN register_teacher rt ON r.Id_Register = rt.Id_Register
						  LEFT JOIN stuff_teacher st ON st.Id_Stuff_Teacher = rt.Id_Stuff_Teacher
						  LEFT JOIN teacher t ON t.Id_Teacher = st.Id_Teacher
                          LEFT JOIN teacher tt ON r.Id_Teacher = tt.Id_Teacher
...
BEGIN
	SELECT 
		...
		IFNULL(c.Name,'No registrada') as Name,
		c.Id_Career
	FROM register r LEFT JOIN register_teacher rt ON r.Id_Register = rt.Id_Register
						  LEFT JOIN stuff_teacher st ON st.Id_Stuff_Teacher = rt.Id_Stuff_Teacher
						  LEFT JOIN teacher t ON t.Id_Teacher = st.Id_Teacher
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `update_register_teacher` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8 */ ;
...
/*!50003 SET sql_mode              = 'NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `update_register_teacher`(IN Id INT, IN IdLab INT, IN IdStu INT, IN IdRegCirc INT, IN NumberStudents INT, IN reason VARCHAR(100))
BEGIN
	UPDATE register r
